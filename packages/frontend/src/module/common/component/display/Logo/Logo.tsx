...
export default function Logo({ className, style, fullSize = false }: LogoProps): JSX.Element {
    const {
        palette: { mode },
    } = useTheme();

    return (
        <Link to="/" type="router" style={{ width: !fullSize ? "2.10rem" : "fit-content", marginRight: !fullSize ? "1rem" : "0" }}>
